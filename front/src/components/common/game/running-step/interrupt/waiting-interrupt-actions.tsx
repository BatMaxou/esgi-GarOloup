...
const WaitingInterruptActions = () => {
  const t = useTranslations('components.common.game.waitingNightActions');
  const { game } = useGame();
  const currentRole = game?.interruptedByRole;
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-8 h-full w-full">
      ...
      <div className="flex flex-col items-center justify-center gap-2">{renderCurrentTurnRoleIndicator()}</div>
    </div>
  );
};

export default WaitingInterruptActions;
