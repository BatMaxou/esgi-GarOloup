...
class ClassicGameDispatchedWithGameMasterAndRandomDispatchStory extends ClassicGameWithGameMasterSettedAndRandomDispatchStory
{
    ...
    public function build(): void
    {
        ...
        $gameMasterBuilder = $this->getState(self::GAME_MASTER);
        ...
        $assignmentIndex = 0;
        foreach ($this->getPool(self::PLAYERS_POOL) as $playerBuilder) {
            \assert($playerBuilder instanceof PlayerBuilder);

            if ($playerBuilder === $gameMasterBuilder) {
                continue;
            }

            [$stateName, $gameRoleBuilder] = $assignments[$assignmentIndex];
            ++$assignmentIndex;
    ...
    {
        return 'classic-game-dispatched-with-game-master-and-random-dispatch-';
    }
}
