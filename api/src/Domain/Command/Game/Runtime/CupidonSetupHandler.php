...
class CupidonSetupHandler
{
    ...
    public function __invoke(CupidonSetupCommand $command): BasicActionOutput
    {
        ...
        $gameEvent = new CupidonSetupEvent()
            ->setGame($player->getLinkedGame())
            ->setUser($currentUser)
            ->setFirstLoverId($command->firstLoverId)
            ->setSecondLoverId($command->secondLoverId);
